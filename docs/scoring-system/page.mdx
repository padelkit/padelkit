...
3.  **Active Server Tracking**: The active server and player can always be dynamically tracked using the `server` property on `MatchScore` (which returns a [`ServingState`](/serving-state) object containing both `team` and `player`).
